...
module.exports = async function groupBenefitYearNull(driver, By, until, Key, fs) {
    ...
    //Insert group
    (await driver).findElement(By.xpath(`//input[@placeholder='Search by Group Number']`)).sendKeys(`${groupName.benefitYearNull}`);
    //wait
    await driverSleep();
    //Insert group type
    ...
    await driverSleep();
    //Insert Benefit Year
    (await driver).findElement(By.name(`groups_benefit_year_start`)).sendKeys(``);
    //wait
    await driverSleep();
